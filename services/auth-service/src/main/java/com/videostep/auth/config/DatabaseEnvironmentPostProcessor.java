...
public class DatabaseEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
                    ...
                    try {
                        ...
                        if (dbUrlProperties.containsKey("SPRING_DATASOURCE_USERNAME")) {
                            properties.put("SPRING_DATASOURCE_USERNAME",
                                    dbUrlProperties.get("SPRING_DATASOURCE_USERNAME"));
                            System.out
                        ...
                        if (dbUrlProperties.containsKey("SPRING_DATASOURCE_PASSWORD")) {
                            properties.put("SPRING_DATASOURCE_PASSWORD",
                                    dbUrlProperties.get("SPRING_DATASOURCE_PASSWORD"));
                            System.out
        ...
        System.out.println("DatabaseEnvironmentPostProcessor: DATABASE_URL (final) = "
                + (databaseUrl != null ? databaseUrl.substring(0, Math.min(50, databaseUrl.length())) + "..."
                        : "null"));
    ...
    private void parsePostgresUrl(String url, Map<String, Object> properties) throws URISyntaxException {
        ...
        if (url.startsWith("postgresql://")) {
            System.out.println(
                    "DatabaseEnvironmentPostProcessor: Converting PostgreSQL URL to MySQL URL in parsePostgresUrl");
        ...
        // JDBC URLを構築（認証情報なし）
        String jdbcUrl = String.format("jdbc:mysql://%s:%d/%s?useSSL=false&allowPublicKeyRetrieval=true", host, port,
                database);
